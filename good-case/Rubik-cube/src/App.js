...
import { zip, dict, permutation } from './components/utils'
...
class App extends Component {
  ...
  transformByOperation(x, y, z) {
    ...
    const map =
      reverse * OPERATIONS[transformAxis][pos].base === -1
        ? TRANSFORM_CORRD_R_MAP
        : TRANSFORM_CORRD_MAP
    ...
    for (let i = 1; i < 4; i++) {
      for (let j = 1; j < 4; j++) {
        for (let k = 1; k < 4; k++) {
          ...
          const { x, y, z, transformAxis } = this.transformByOperation(
            i1,
            j1,
            k1
          )
          ...
          for (const axis of 'xyz') {
            ...
            if (newDir === operate) {
              transform[i][j][k][
                `rotate${info[axis][0].toUpperCase()}`
              ] += rotateInc
            }
          }
